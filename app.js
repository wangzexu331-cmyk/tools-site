...
const GOVERNANCE_REVISION_DEFAULT_FIELDS = [
  "seq",
  "hazardNo",
  "checkDate",
  "checkPlace",
...
const GOVERNANCE_REVISION_BULK_EXCLUDED_FIELDS = new Set([
  "seq",
  "checkType",
  "checkNo",
  "hazardNo",
...
const HAZARD_TEXT_CODE_PREFIX = "HAZARD-DATA:v1:";
const HAZARD_TEXT_CODE_SINGLE_LIMIT = 30 * 1024;
const HAZARD_TEXT_CODE_PART_SIZE = 20 * 1024;
...
const governanceState = {
  ...
  feedbackSubMode: "import",
  hazardImportDraft: null,
  hazardEntryMode: "inspectionJson",
  draggingJsonSourceId: "",
  dashboardMetric: "",
  dashboardDeptFilter: "",
  revisionCheckTypeCode: "",
  revisionFields: GOVERNANCE_REVISION_DEFAULT_FIELDS,
  revisionFieldSettingsOpen: false,
  revisionBulkOpen: false,
  revisionSelectedHazardIds: [],
...
function sanitizePhotoReference(photo = {}, type = "") {
  return {
    id: photo.id || "",
    fileName: photo.fileName || photo.name || photo.originalName || "",
    originalName: photo.originalName || photo.name || "",
    hazardNo: photo.hazardNo || "",
    type: photo.type || type || "",
    remark: photo.remark || "",
    createTime: photo.createTime || photo.createdAt || "",
    mimeType: photo.mimeType || ""
  };
}

function stripPhotoPayloadForTextCode(value, key = "") {
  if (Array.isArray(value)) {
    if (["beforePhotos", "afterPhotos", "photos"].includes(key)) {
      const type = key === "beforePhotos" ? "before" : key === "afterPhotos" ? "after" : "";
      return value.map((item) => typeof item === "object" && item ? sanitizePhotoReference(item, type) : item);
    }
    return value.map((item) => stripPhotoPayloadForTextCode(item));
  }
  if (!value || typeof value !== "object") return value;
  const output = {};
  Object.entries(value).forEach(([childKey, childValue]) => {
    if (["dataUrl", "watermarkedDataUrl", "originalDataUrl", "base64", "preview", "blob", "file", "bytes"].includes(childKey)) {
      return;
    }
    output[childKey] = stripPhotoPayloadForTextCode(childValue, childKey);
  });
  return output;
}

function inferHazardTextCodeDataType(data = {}) {
  if (data.schemaVersion === "hazard-feedback-v1" || data.exportType === "rectification-feedback") return "hazard-feedback";
  if (data.schemaVersion === "hazard-rectification-v1" || data.exportType === "rectification-task") return "rectification-task";
  if (data.schemaVersion === "hazard-governance-full-backup-v1" || data.exportType === "governance-full-backup") return "governance-full-backup";
  if (data.inspection && Array.isArray(data.hazards)) return "inspection-create";
  if (data.checkInfo && Array.isArray(data.hazards)) return data.dataType || "hazard-data";
  return data.dataType || "hazard-data";
}

function buildHazardTextCodeCheckInfo(data = {}) {
  const inspection = data.checkInfo || data.inspection || {};
  return {
    checkNo: inspection.checkNo || data.checkNo || "",
    checkName: inspection.checkName || data.checkName || "",
    checkDate: inspection.checkDate || data.checkDate || inspection.date || "",
    checkPlace: inspection.checkPlace || data.checkPlace || inspection.location || "",
    checkType: inspection.checkType || data.checkType || "",
    leader: inspection.leader || inspection.groupLeader || data.groupLeader || ""
  };
}

function prepareHazardTextCodePayload(jsonData, options = {}) {
  const payload = stripPhotoPayloadForTextCode(JSON.parse(JSON.stringify(jsonData || {})));
  payload.dataType = options.dataType || inferHazardTextCodeDataType(payload);
  payload.version = payload.version || "1.0";
  payload.originalExportType = payload.originalExportType || payload.exportType || "";
  payload.exportType = "text-code";
  payload.exportTime = payload.exportTime || new Date().toISOString();
  payload.checkInfo = payload.checkInfo || buildHazardTextCodeCheckInfo(payload);
  return payload;
}
...
async function gzipStringToBase64Url(text) {
  ...
  try {
    ...
    console.warn("数据码压缩失败，改用普通编码", error);
    return "";
  }
}
...
async function ungzipBase64UrlToString(encoded) {
  if (!("DecompressionStream" in window)) {
    throw new Error("当前浏览器不支持解析压缩数据码，请使用新版 Chrome 或 Edge。");
  }
...
async function compressJsonToHazardTextCodeData(jsonText) {
  const compressed = await gzipStringToBase64Url(jsonText);
  if (compressed) return compressed;
  return `j.${bytesToBase64Url(new TextEncoder().encode(jsonText))}`;
}

async function decompressHazardTextCodeData(encoded) {
  if (encoded.startsWith("gz.")) return ungzipBase64UrlToString(encoded.slice(3));
  if (encoded.startsWith("j.")) return new TextDecoder().decode(base64UrlToBytes(encoded.slice(2)));
  throw new Error("数据码解析失败，可能复制不完整，请重新复制完整数据码。");
}

function getHazardTextCodeLabel(dataType) {
  if (dataType === "hazard-feedback") return "隐患整改反馈数据";
  if (dataType === "rectification-task") return "隐患整改任务数据";
  if (dataType === "governance-full-backup") return "隐患治理备份数据";
  if (dataType === "inspection-create") return "隐患检查数据";
  return "隐患数据";
}

function splitTextCode(encoded, maxSize = HAZARD_TEXT_CODE_PART_SIZE) {
  const parts = [];
  for (let index = 0; index < encoded.length; index += maxSize) {
    parts.push(encoded.slice(index, index + maxSize));
  }
  return parts;
}

async function encodeHazardDataToTextCode(jsonData, options = {}) {
  const payload = prepareHazardTextCodePayload(jsonData, options);
  const jsonText = JSON.stringify(payload);
  const encoded = await compressJsonToHazardTextCodeData(jsonText);
  const label = options.label || getHazardTextCodeLabel(payload.dataType);
  const checkNo = safeFilePart(payload.checkInfo?.checkNo || payload.checkNo || "NO-CHECK");
  const single = `【${label}】\n${HAZARD_TEXT_CODE_PREFIX}${encoded}`;
  if (single.length <= HAZARD_TEXT_CODE_SINGLE_LIMIT) return single;
  const parts = splitTextCode(encoded);
  return parts.map((part, index) => `【${label} ${index + 1}/${parts.length}】\n${HAZARD_TEXT_CODE_PREFIX}part:${index + 1}/${parts.length}:${checkNo}:${part}`).join("\n\n");
}

function mergeTextCodeParts(matches) {
  const total = Number(matches[0]?.[2] || 0);
  if (!total || matches.some((match) => Number(match[2]) !== total)) {
    throw new Error("分包数据码格式不一致，请检查是否复制完整。");
  }
  const byIndex = new Map();
  matches.forEach((match) => {
    byIndex.set(Number(match[1]), match[4]);
  });
  const missing = [];
  ...
  if (missing.length) {
    throw new Error(`当前分包数据不完整，缺少第 ${missing.join("、")} 包。`);
  }
  return Array.from({ length: total }, (_, index) => byIndex.get(index + 1)).join("");
}

async function decodeHazardTextCode(text) {
  const raw = String(text || "").trim();
  if (!raw.includes("HAZARD-DATA")) {
    throw new Error("未识别到 HAZARD-DATA 数据码，请检查是否复制完整。");
  }
  if (!raw.includes(HAZARD_TEXT_CODE_PREFIX)) {
    throw new Error("当前数据码版本暂不支持，请使用最新平台重新导出。");
  }
  const partMatches = Array.from(raw.matchAll(/HAZARD-DATA:v1:part:(\d+)\/(\d+):([^:\s]+):([A-Za-z0-9_.-]+)/g));
  let encoded = "";
  let partInfo = null;
  if (partMatches.length) {
    encoded = mergeTextCodeParts(partMatches);
    partInfo = { current: partMatches.length, total: Number(partMatches[0][2]) };
  } else {
    const singleMatch = raw.match(/HAZARD-DATA:v1:([A-Za-z0-9_.-]+)/);
    if (!singleMatch) throw new Error("数据码解析失败，可能复制不完整，请重新复制完整数据码。");
    encoded = singleMatch[1];
  }
  try {
    const jsonText = await decompressHazardTextCodeData(encoded);
    const data = JSON.parse(jsonText);
    return { data, version: "v1", partInfo };
  } catch (error) {
...
function validateHazardImportData(jsonData, expectedTypes = []) {
  const dataType = inferHazardTextCodeDataType(jsonData);
  if (expectedTypes.length && !expectedTypes.includes(dataType)) {
    throw new Error(`当前数据码类型为“${dataType}”，不能导入到此页面。`);
  }
  const hazards = Array.isArray(jsonData.hazards) ? jsonData.hazards : Array.isArray(jsonData.data?.hazards) ? jsonData.data.hazards : [];
  const checkInfo = jsonData.checkInfo || buildHazardTextCodeCheckInfo(jsonData);
  if (dataType !== "governance-full-backup" && !checkInfo && !jsonData.checkNo) {
    throw new Error("数据中缺少 checkInfo，无法导入。");
  }
  if (dataType !== "governance-full-backup" && !Array.isArray(hazards)) {
    throw new Error("数据中缺少 hazards，无法导入。");
  }
  return { dataType, hazards, checkInfo };
}

function previewImportData(jsonData) {
  const { dataType, hazards, checkInfo } = validateHazardImportData(jsonData);
  const statusValue = (hazard) => hazard.currentStatus || hazard.status || hazard.autoStatus || "";
  return {
    dataType,
    checkName: checkInfo.checkName || jsonData.checkName || "",
    checkNo: checkInfo.checkNo || jsonData.checkNo || "",
    checkDate: checkInfo.checkDate || jsonData.checkDate || "",
    checkPlace: checkInfo.checkPlace || jsonData.checkPlace || "",
    checkType: checkInfo.checkType || jsonData.checkType || "",
    responsibleDept: jsonData.responsibleDept || "",
    total: hazards.length,
    done: hazards.filter((hazard) => statusValue(hazard) === "已整改").length,
    unfinished: hazards.filter((hazard) => statusValue(hazard) !== "已整改").length,
    delayed: hazards.filter((hazard) => statusValue(hazard) === "延期整改").length,
    overdue: hazards.filter((hazard) => statusValue(hazard) === "已逾期").length,
    source: "文本数据码",
    version: "v1"
  };
}

function renderTextCodePreview(preview, options = {}) {
  return `
    <div class="text-code-preview">
      <div class="report-summary-strip">
        <span><b>${escapeHtml(preview.total)}</b> 隐患总数</span>
        <span><b>${escapeHtml(preview.done)}</b> 已整改</span>
        <span><b>${escapeHtml(preview.unfinished)}</b> 未整改</span>
        <span><b>${escapeHtml(preview.delayed)}</b> 延期整改</span>
        <span><b>${escapeHtml(preview.overdue)}</b> 逾期</span>
      </div>
      <dl class="text-code-preview-grid">
        <div><dt>检查名称</dt><dd>${escapeHtml(preview.checkName || "未填写")}</dd></div>
        <div><dt>检查编号</dt><dd>${escapeHtml(preview.checkNo || "未填写")}</dd></div>
        <div><dt>检查日期</dt><dd>${escapeHtml(preview.checkDate || "未填写")}</dd></div>
        <div><dt>检查地点</dt><dd>${escapeHtml(preview.checkPlace || "未填写")}</dd></div>
        <div><dt>检查类型</dt><dd>${escapeHtml(preview.checkType || "未填写")}</dd></div>
        <div><dt>责任部门</dt><dd>${escapeHtml(preview.responsibleDept || "未填写")}</dd></div>
        <div><dt>数据来源</dt><dd>${escapeHtml(preview.source)}</dd></div>
        <div><dt>数据版本</dt><dd>${escapeHtml(preview.version)}</dd></div>
      </dl>
      <div class="actions">
        <button class="button primary" type="button" data-text-code-confirm="${escapeAttr(options.confirmAction || "")}">确认导入</button>
        <button class="button ghost" type="button" data-text-code-cancel="${escapeAttr(options.cancelAction || "")}">取消</button>
      </div>
      <details class="text-code-raw"><summary>查看原始数据</summary><pre>${escapeHtml(JSON.stringify(options.rawData || {}, null, 2))}</pre></details>
    </div>
  `;
}

async function copyHazardTextCode(jsonData, options = {}) {
  const code = await encodeHazardDataToTextCode(jsonData, options);
  await copyText(code);
  showToast("已复制微信数据码，可直接粘贴到微信发送。对方打开平台后，在“数据码导入”中粘贴即可导入。");
  return code;
}

async function showHazardTextCodeDialog(jsonData, options = {}) {
  const code = await encodeHazardDataToTextCode(jsonData, options);
  document.querySelector("#hazardTextCodeDialog")?.remove();
  const dialog = document.createElement("div");
  dialog.id = "hazardTextCodeDialog";
  dialog.className = "text-code-dialog";
  dialog.innerHTML = `
    <div class="text-code-dialog-card glass">
      <div class="section-title">
        <div>
          <h3>${escapeHtml(options.title || "微信数据码")}</h3>
          <p class="hint">数据码不包含照片本体，照片请单独发送或通过照片功能导入。</p>
        </div>
        <button class="icon-button" type="button" data-close-text-code>×</button>
      </div>
      <textarea class="textarea text-code-output" readonly>${escapeHtml(code)}</textarea>
      <div class="actions">
        <button class="button primary" type="button" data-copy-text-code>复制微信数据码</button>
        <button class="button" type="button" data-close-text-code>关闭</button>
      </div>
    </div>
  `;
  document.body.appendChild(dialog);
  dialog.querySelectorAll("[data-close-text-code]").forEach((button) => button.addEventListener("click", () => dialog.remove()));
  dialog.querySelector("[data-copy-text-code]").addEventListener("click", () => copyText(code).then(() => showToast("数据码已复制")));
}
...
async function renderRectificationFeedbackTool() {
  ...
  app.innerHTML = `
    <div class="tool-layout governance-module">
      <section class="tool-card glass governance-hero">
        ...
        <div class="actions">
          <button id="importRectificationTaskButton" class="button" type="button">选择 JSON 文件导入</button>
          <button id="copyRectificationFeedbackCodeButton" class="button primary" type="button">复制微信数据码</button>
          <button id="viewRectificationFeedbackCodeButton" class="button" type="button">查看数据码</button>
          <button id="exportRectificationFeedbackButton" class="button" type="button">下载 JSON 文件</button>
          <button class="button ghost" type="button" disabled>生成二维码（预留）</button>
          <input id="rectificationTaskJsonInput" class="hidden" type="file" accept="application/json,.json">
        </div>
      </section>
      <section class="tool-card glass text-code-panel">
        <div class="section-title">
          <div>
            <h3>推荐方式：粘贴微信数据码</h3>
            <p class="hint">从微信中复制收到的数据码，粘贴到下方即可导入。数据码不包含照片本体，照片请单独发送或通过照片功能导入。</p>
          </div>
        </div>
        <label class="field span-2">
          <span>数据码导入</span>
          <textarea id="rectificationTaskTextCodeInput" class="textarea text-code-input" placeholder="请粘贴微信中收到的隐患数据码，格式一般为：&#10;【隐患整改任务数据】&#10;HAZARD-DATA:v1:......"></textarea>
        </label>
        <div class="actions">
          <button id="parseRectificationTaskTextCodeButton" class="button primary" type="button">识别数据码</button>
          <button id="clearRectificationTaskTextCodeButton" class="button ghost" type="button">清空</button>
        </div>
        <div id="rectificationTaskTextCodePreview" class="text-code-preview-panel"></div>
      </section>
  ...
  document.querySelector("#copyRectificationFeedbackCodeButton").addEventListener("click", copyStandaloneRectificationFeedbackTextCode);
  document.querySelector("#viewRectificationFeedbackCodeButton").addEventListener("click", viewStandaloneRectificationFeedbackTextCode);
  document.querySelector("#parseRectificationTaskTextCodeButton").addEventListener("click", previewStandaloneRectificationTaskTextCode);
  document.querySelector("#clearRectificationTaskTextCodeButton").addEventListener("click", () => {
    document.querySelector("#rectificationTaskTextCodeInput").value = "";
    document.querySelector("#rectificationTaskTextCodePreview").innerHTML = "";
  });
...
async function previewStandaloneRectificationTaskTextCode() {
  const input = document.querySelector("#rectificationTaskTextCodeInput");
  const panel = document.querySelector("#rectificationTaskTextCodePreview");
  const text = input?.value || "";
  try {
    const decoded = await decodeHazardTextCode(text);
    validateHazardImportData(decoded.data, ["rectification-task"]);
    const preview = previewImportData(decoded.data);
    panel.innerHTML = renderTextCodePreview(preview, {
      confirmAction: "rectification-task",
      cancelAction: "rectification-task",
      rawData: decoded.data
    });
    panel.querySelector("[data-text-code-confirm]").addEventListener("click", () => importStandaloneRectificationTaskTextCode(decoded.data));
    panel.querySelector("[data-text-code-cancel]").addEventListener("click", () => {
      panel.innerHTML = "";
    });
  } catch (error) {
    panel.innerHTML = "";
    showToast(error.message || "数据码解析失败");
  }
}

async function importStandaloneRectificationTaskTextCode(data) {
  try {
    const draftData = normalizeGovernanceData(JSON.parse(JSON.stringify(governanceState.data)));
    const result = importRectificationTaskIntoGovernanceData(draftData, data);
    if (!window.confirm(`将导入整改任务：${result.checkNo}，共 ${result.total} 条隐患。\n新增 ${result.created} 条，更新 ${result.updated} 条，是否继续？`)) {
    ...
    await saveRectificationTaskData();
    governanceState.selectedInspectionId = result.inspectionId;
    showToast("整改任务数据码已导入");
    await renderRectificationFeedbackTool();
  } catch (error) {
    showToast(error.message || "整改任务数据码导入失败");
  }
}
...
function exportStandaloneRectificationFeedbackJson() {
  const data = buildStandaloneRectificationFeedbackPayload();
  if (!data) return;
...
function buildStandaloneRectificationFeedbackPayload() {
...
async function copyStandaloneRectificationFeedbackTextCode() {
  const data = buildStandaloneRectificationFeedbackPayload();
  if (!data) return;
  try {
    await copyHazardTextCode(data, { dataType: "hazard-feedback", label: "隐患整改反馈数据" });
  } catch (error) {
    showToast(error.message || "数据码复制失败");
  }
}

async function viewStandaloneRectificationFeedbackTextCode() {
  const data = buildStandaloneRectificationFeedbackPayload();
  if (!data) return;
  try {
    await showHazardTextCodeDialog(data, { title: "整改反馈微信数据码", dataType: "hazard-feedback", label: "隐患整改反馈数据" });
  } catch (error) {
    showToast(error.message || "数据码生成失败");
  }
}
...
function renderGovernanceTaskExportPage(content) {
  ...
  content.innerHTML = `
    <section class="tool-card glass governance-sticky-panel">
      ...
      <div class="compact-form">
        ...
        <button id="copyTaskTextCodeButton" class="button primary" type="button">复制微信数据码</button>
        <button id="viewTaskTextCodeButton" class="button" type="button">查看数据码</button>
        <button id="exportTaskJsonButton" class="button" type="button">下载 JSON 文件</button>
        <button class="button ghost" type="button" disabled>生成二维码（预留）</button>
      </div>
      <p class="hint">推荐使用：复制微信数据码。适合通过微信发送文字数据，不需要下载文件，不需要查找文件夹。注意：数据码不包含照片本体，照片请单独发送或通过照片功能导入。</p>
    </section>
    <section class="tool-card glass">
      <div class="section-title">
        <div>
  ...
  content.querySelector("#copyTaskTextCodeButton").addEventListener("click", copyGovernanceTaskTextCode);
  content.querySelector("#viewTaskTextCodeButton").addEventListener("click", viewGovernanceTaskTextCode);
}
...
function exportGovernanceTaskJson() {
  const data = buildGovernanceTaskJsonPayload();
  if (!data) return;
  downloadTextFile(`${data.checkNo}-${safeFilePart(data.responsibleDept || "全部责任部门")}-整改任务.json`, JSON.stringify(data, null, 2), "application/json;charset=utf-8");
  showToast("整改任务 JSON 已导出");
}

function buildGovernanceTaskJsonPayload() {
  const inspection = getSelectedGovernanceInspection();
  if (!inspection) {
    showToast("请先选择检查任务");
    return null;
  ...
  return {
    ...
    checkPlace: inspection.checkPlace,
    checkType: inspection.checkType,
    groupLeader: inspection.groupLeader,
...
async function copyGovernanceTaskTextCode() {
  const data = buildGovernanceTaskJsonPayload();
  if (!data) return;
  try {
    await copyHazardTextCode(data, { dataType: "rectification-task", label: "隐患整改任务数据" });
  } catch (error) {
    showToast(error.message || "数据码复制失败");
  }
}

async function viewGovernanceTaskTextCode() {
  const data = buildGovernanceTaskJsonPayload();
  if (!data) return;
  try {
    await showHazardTextCodeDialog(data, { title: "整改任务微信数据码", dataType: "rectification-task", label: "隐患整改任务数据" });
  } catch (error) {
    showToast(error.message || "数据码生成失败");
  }
}
...
function renderGovernanceFeedbackImportPage(content) {
  const isRevisionMode = governanceState.feedbackSubMode === "revision";
  content.innerHTML = `
    <section class="tool-card glass">
      <div class="section-title">
        <div>
          <h2>整改汇总</h2>
          <p class="hint">可导入整改反馈 JSON 核对后提交，也可以按检查任务集中修订隐患状态、责任信息和闭环信息。</p>
        </div>
      </div>
      <div class="actions submode-actions">
        <button class="button ${!isRevisionMode ? "primary" : ""}" type="button" data-feedback-submode="import">整改反馈导入</button>
        <button class="button ${isRevisionMode ? "primary" : ""}" type="button" data-feedback-submode="revision">检查任务全面修订</button>
      </div>
    </section>
    ${isRevisionMode ? renderGovernanceTaskRevisionPage() : `
    <section class="tool-card glass text-code-panel">
      <div class="section-title">
        <div>
          <h3>方式一：粘贴微信数据码导入（推荐）</h3>
          <p class="hint">从微信中复制收到的整改反馈数据码，粘贴到下方即可导入。解析成功后会先显示预览，不会直接写入台账。</p>
        </div>
      </div>
      <label class="field span-2">
        <span>整改反馈数据码</span>
        <textarea id="feedbackTextCodeInput" class="textarea text-code-input" placeholder="请粘贴微信中收到的隐患数据码，格式一般为：&#10;【隐患整改反馈数据】&#10;HAZARD-DATA:v1:......"></textarea>
      </label>
      <div class="actions">
        <button id="parseFeedbackTextCodeButton" class="button primary" type="button">识别数据码</button>
        <button id="clearFeedbackTextCodeButton" class="button ghost" type="button">清空</button>
      </div>
      <div id="feedbackTextCodePreviewPanel" class="text-code-preview-panel"></div>
    </section>
    <section class="tool-card glass">
      <div class="section-title"><h3>方式二：选择 JSON 文件导入（备用）</h3><p class="hint">适合电脑端或能方便找到文件的手机环境。导入整改反馈 JSON 后先核对，不会立即写入台账。</p></div>
      <div class="actions">
    ...
    `}
  `;
  content.querySelectorAll("[data-feedback-submode]").forEach((button) => {
    button.addEventListener("click", () => {
      governanceState.feedbackSubMode = button.dataset.feedbackSubmode || "import";
      renderGovernanceActiveTab();
    });
  });
  if (isRevisionMode) {
    bindGovernanceTaskRevisionEvents(content);
    return;
  }
  content.querySelector("#parseFeedbackTextCodeButton")?.addEventListener("click", previewGovernanceFeedbackTextCode);
  content.querySelector("#clearFeedbackTextCodeButton")?.addEventListener("click", () => {
    content.querySelector("#feedbackTextCodeInput").value = "";
    content.querySelector("#feedbackTextCodePreviewPanel").innerHTML = "";
  });
...
function getGovernanceRevisionFields() {
  const allowed = new Set(GOVERNANCE_LEDGER_FIELDS.map((field) => field.key));
  const fields = Array.isArray(governanceState.revisionFields)
    ? governanceState.revisionFields.filter((key) => allowed.has(key))
    : [];
  return fields.length ? fields : GOVERNANCE_REVISION_DEFAULT_FIELDS;
}

function getSelectedGovernanceRevisionFields() {
  const keys = getGovernanceRevisionFields();
  return keys.map((key) => GOVERNANCE_LEDGER_FIELDS.find((field) => field.key === key)).filter(Boolean);
}

function getGovernanceRevisionBulkFields() {
  return getSelectedGovernanceRevisionFields().filter((field) => !GOVERNANCE_REVISION_BULK_EXCLUDED_FIELDS.has(field.key));
}

function getGovernanceTaskRevisionInspection() {
  const inspections = governanceState.data.inspections || [];
  if (!inspections.length) return null;
  const selected = inspections.find((item) => item.id === governanceState.selectedInspectionId);
  const typeCode = governanceState.revisionCheckTypeCode || selected?.checkTypeCode || inspections[0].checkTypeCode || "NB";
  governanceState.revisionCheckTypeCode = typeCode;
  const typed = inspections.filter((item) => item.checkTypeCode === typeCode);
  if (!typed.length) {
    governanceState.selectedInspectionId = "";
    return null;
  }
  const inspection = typed.find((item) => item.id === governanceState.selectedInspectionId) || typed[0];
  governanceState.selectedInspectionId = inspection?.id || "";
  return inspection || null;
}

function getGovernanceTaskRevisionHazards(inspection = getGovernanceTaskRevisionInspection()) {
  if (!inspection) return [];
  return governanceState.data.hazards.filter((hazard) => hazard.checkNo === inspection.checkNo);
}

function getGovernanceTaskRevisionStats(hazards) {
  const total = hazards.length;
  const done = hazards.filter((item) => item.autoStatus === "已整改" || item.currentStatus === "已整改").length;
  const delayed = hazards.filter((item) => item.currentStatus === "延期整改" || item.autoStatus === "延期整改").length;
  const overdue = hazards.filter((item) => item.autoStatus === "已逾期").length;
  const closed = hazards.filter((item) => item.platformCloseStatus === "已上传闭环").length;
  return {
    total,
    done,
    unfinished: Math.max(total - done, 0),
    delayed,
    overdue,
    closed,
    unclosed: Math.max(total - closed, 0)
  };
}

function renderGovernanceTaskRevisionPage() {
  const inspection = getGovernanceTaskRevisionInspection();
  const inspections = governanceState.data.inspections || [];
  if (!inspections.length) {
    return `<section class="tool-card glass"><div class="empty-state soft"><h3>暂无检查任务</h3><p>请先在“检查任务”中新建检查。</p></div></section>`;
  }
  const typeCode = governanceState.revisionCheckTypeCode || inspection?.checkTypeCode || "NB";
  const typedInspections = inspections.filter((item) => item.checkTypeCode === typeCode);
  const hazards = getGovernanceTaskRevisionHazards(inspection);
  const stats = getGovernanceTaskRevisionStats(hazards);
  const allClosed = hazards.length > 0 && stats.closed === hazards.length;
  const allDone = hazards.length > 0 && stats.done === hazards.length;
  return `
    <section class="tool-card glass revision-workbench">
      <div class="section-title">
        <div>
          <h3>检查任务全面修订</h3>
          <p class="hint">选择一项检查任务后，可统一修订闭环状态、整改状态、责任信息和各隐患明细。</p>
        </div>
      </div>
      <div class="revision-task-select-grid">
        <label class="field"><span>检查类型</span><select id="revisionCheckTypeCode" class="select">${governanceState.data.settings.checkTypes.map((type) => `<option value="${escapeAttr(type.code)}" ${type.code === typeCode ? "selected" : ""}>${escapeHtml(type.name)}</option>`).join("")}</select></label>
        <label class="field"><span>选择检查任务</span><select id="revisionInspectionId" class="select">${typedInspections.length ? typedInspections.map((item) => `<option value="${escapeAttr(item.id)}" ${item.id === inspection?.id ? "selected" : ""}>${escapeHtml(item.checkNo)} ${escapeHtml(formatGovernanceInspectionDateRange(item))} ${escapeHtml(item.checkName || "未命名检查")}</option>`).join("") : `<option value="">该类型暂无检查任务</option>`}</select></label>
      </div>
      <div class="report-summary-strip revision-stat-strip">
        <span><b>${stats.total}</b> 隐患总数</span>
        <span><b>${stats.done}</b> 已整改</span>
        <span><b>${stats.unfinished}</b> 未整改</span>
        <span><b>${stats.overdue}</b> 已逾期</span>
        <span><b>${stats.delayed}</b> 延期整改</span>
        <span><b>${stats.closed}</b> 已上传闭环</span>
        <span><b>${stats.unclosed}</b> 未上传闭环</span>
      </div>
      <div class="revision-quick-actions">
        <button id="revisionMarkClosedButton" class="button ${allClosed ? "ghost" : "primary"}" type="button">${allClosed ? "已全部上传闭环" : "标记全部已上传闭环"}</button>
        <button id="revisionMarkDoneButton" class="button ${allDone ? "ghost" : "primary"}" type="button">${allDone ? "已全部完成整改" : "标记全部完成整改"}</button>
      </div>
    </section>
    ${renderGovernanceRevisionFieldSettings()}
    ${renderGovernanceRevisionBulkPanel(hazards)}
    ${renderGovernanceRevisionEditableTable(hazards)}
  `;
}

function renderGovernanceRevisionFieldSettings() {
  const fields = new Set(getGovernanceRevisionFields());
  return `
    <details class="tool-card glass ledger-field-settings revision-field-settings" ${governanceState.revisionFieldSettingsOpen ? "open" : ""}>
      <summary>
        <strong>字段选择</strong>
        <small>选择下方修订表要展示的信息，批量修改会跟随这里的字段显示。</small>
      </summary>
      <div class="field-check-grid">
        ${GOVERNANCE_LEDGER_FIELDS.map((field) => `
          <label class="field-check">
            <input type="checkbox" value="${escapeAttr(field.key)}" data-revision-field ${fields.has(field.key) ? "checked" : ""}>
            <span>${escapeHtml(field.label)}</span>
          </label>
        `).join("")}
      </div>
...
function renderGovernanceRevisionBulkPanel(hazards) {
  const selected = new Set(governanceState.revisionSelectedHazardIds || []);
  const bulkFields = getGovernanceRevisionBulkFields();
  return `
    <details class="tool-card glass revision-bulk-panel" ${governanceState.revisionBulkOpen ? "open" : ""}>
      <summary>
        <strong>批量修改</strong>
        <small>只对选中的隐患生效；空字段不会覆盖原内容。</small>
      </summary>
      <div class="revision-index-toolbar">
        <strong>选择隐患序号</strong>
        <div class="actions">
          <button class="button mini" type="button" data-revision-select="all">全选</button>
          <button class="button mini" type="button" data-revision-select="invert">反选</button>
          <button class="button mini" type="button" data-revision-select="clear">清空</button>
        </div>
      </div>
      <div class="hazard-number-selector revision-number-selector">
        ${hazards.map((hazard, index) => `<button class="hazard-number ${selected.has(hazard.id) ? "active" : ""}" type="button" data-revision-toggle-id="${escapeAttr(hazard.id)}">${index + 1}</button>`).join("") || `<span class="hint">当前检查暂无隐患</span>`}
      </div>
      <div class="form-grid revision-bulk-grid">
        ${bulkFields.length ? bulkFields.map((field) => renderGovernanceRevisionBulkField(field)).join("") : `<p class="hint span-2">当前展示字段中没有可批量修改的字段。</p>`}
      </div>
      <div class="actions">
        <button id="revisionApplyBulkButton" class="button primary" type="button">应用到所选隐患</button>
      </div>
    </details>
  `;
}

function renderGovernanceRevisionBulkField(field) {
  const key = field.key;
  const settings = governanceState.data.settings;
  if (key === "hazardLevel") {
    return `<label class="field"><span>${escapeHtml(field.label)}</span><select class="select" data-revision-bulk-field="${key}"><option value="">不修改</option>${settings.hazardLevels.map((name) => `<option>${escapeHtml(name)}</option>`).join("")}</select></label>`;
  }
  if (key === "hazardType") {
    return `<label class="field"><span>${escapeHtml(field.label)}</span><select id="revisionBulkHazardType" class="select" data-revision-bulk-field="${key}"><option value="">不修改</option>${Object.keys(settings.hazardTypes || {}).map((name) => `<option>${escapeHtml(name)}</option>`).join("")}</select></label>`;
  }
  if (key === "professionalType") {
    const firstType = Object.keys(settings.hazardTypes || {})[0] || "";
    const options = [...new Set(Object.values(settings.hazardTypes || {}).flat())];
    return `<label class="field"><span>${escapeHtml(field.label)}</span><select id="revisionBulkProfessionalType" class="select" data-revision-bulk-field="${key}"><option value="">不修改</option>${(options.length ? options : (settings.hazardTypes[firstType] || [])).map((name) => `<option>${escapeHtml(name)}</option>`).join("")}</select></label>`;
  }
  if (key === "currentStatus") {
    return `<label class="field"><span>${escapeHtml(field.label)}</span><select class="select" data-revision-bulk-field="${key}"><option value="">不修改</option>${settings.statuses.map((name) => `<option>${escapeHtml(name)}</option>`).join("")}</select></label>`;
  }
  if (key === "platformCloseStatus") {
    return `<label class="field"><span>${escapeHtml(field.label)}</span><select class="select" data-revision-bulk-field="${key}"><option value="">不修改</option>${settings.platformCloseStatuses.map((name) => `<option>${escapeHtml(name)}</option>`).join("")}</select></label>`;
  }
  if (["deadline", "extensionDeadline", "platformUploadDate", "checkDate"].includes(key)) {
    return `<label class="field"><span>${escapeHtml(field.label)}</span><input class="input" type="date" data-revision-bulk-field="${key}"></label>`;
  }
  if (["handlingMeasures", "extensionReason", "platformRemark"].includes(key)) {
    return `<label class="field span-2"><span>${escapeHtml(field.label)}</span><textarea class="textarea small-textarea" data-revision-bulk-field="${key}" placeholder="不填写则不修改"></textarea></label>`;
  }
  const listAttr = key === "responsibleDept" ? ` list="govResponsibleDeptOptions"` : key === "responsiblePerson" ? ` list="govResponsiblePersonOptions"` : key === "supervisionLeader" ? ` list="govSupervisionLeaderOptions"` : key === "acceptor" ? ` list="govAcceptorOptions"` : "";
  return `<label class="field"><span>${escapeHtml(field.label)}</span><input class="input" ${listAttr} data-revision-bulk-field="${key}" placeholder="不填写则不修改"></label>`;
}

function renderGovernanceRevisionEditableTable(hazards) {
  if (!hazards.length) {
    return `<div class="empty-state glass"><h3>当前检查暂无隐患</h3><p>可以先到“隐患汇总”导入或录入隐患。</p></div>`;
  }
  const fields = getSelectedGovernanceRevisionFields();
  const selected = new Set(governanceState.revisionSelectedHazardIds || []);
  const tableMinWidth = Math.max(860, fields.length * 132 + 92);
  return `
    <section class="tool-card glass revision-table-card">
      ${renderGovernanceInlineDatalists()}
      <div class="governance-table-wrap">
        <table class="governance-table revision-edit-table" style="min-width:${tableMinWidth}px">
          <thead>
            <tr>
              <th class="field-seq">选择</th>
              ${fields.map((field) => `<th class="${escapeAttr(field.className || "")}">${escapeHtml(field.label)}</th>`).join("")}
            </tr>
          </thead>
          <tbody>
            ${hazards.map((hazard, index) => `
              <tr data-revision-hazard-id="${escapeAttr(hazard.id)}" class="${selected.has(hazard.id) ? "row-selected" : ""}">
                <td class="field-seq"><input class="revision-row-select" type="checkbox" value="${escapeAttr(hazard.id)}" ${selected.has(hazard.id) ? "checked" : ""} aria-label="选择第${index + 1}条隐患"></td>
                ${fields.map((field) => `<td class="${escapeAttr(field.className || "")}">${renderGovernanceRevisionCell(field, hazard, index)}</td>`).join("")}
              </tr>
            `).join("")}
          </tbody>
        </table>
...
function renderGovernanceRevisionCell(field, hazard, index) {
  const value = getGovernanceLedgerFieldValue(field, hazard, index);
  const key = field.key;
  const base = `data-revision-hazard-id="${escapeAttr(hazard.id)}" data-revision-inline-field="${escapeAttr(key)}"`;
  const settings = governanceState.data.settings;
  if (key === "seq") return String(index + 1);
  if (key === "checkType" || key === "checkNo" || key === "autoStatus" || key === "beforePhotoCount" || key === "afterPhotoCount" || key === "createdAt" || key === "updatedAt") {
    if (key === "autoStatus") return `<span class="status-pill ${getGovernanceStatusClass(value)}">${escapeHtml(value)}</span>`;
    return escapeHtml(value);
  }
  if (["problem", "rectificationMeasures", "handlingMeasures", "extensionReason", "platformRemark"].includes(key)) {
    return `<textarea class="textarea ledger-inline-control ledger-inline-textarea revision-inline-control" ${base}>${escapeHtml(value || "")}</textarea>`;
  }
  if (["checkDate", "deadline", "extensionDeadline", "platformUploadDate"].includes(key)) {
    const editField = key === "deadline" ? getGovernanceLedgerDeadlineEditField(hazard) : key;
    const editValue = key === "deadline" ? (hazard[editField] || "") : (hazard[key] || "");
    return `<input class="input ledger-inline-control ledger-inline-date revision-inline-control" type="date" value="${escapeAttr(editValue)}" data-revision-hazard-id="${escapeAttr(hazard.id)}" data-revision-inline-field="${escapeAttr(editField)}">`;
  }
  if (key === "hazardLevel") {
    return `<select class="select ledger-inline-control revision-inline-control" ${base}>${settings.hazardLevels.map((name) => `<option ${hazard.hazardLevel === name ? "selected" : ""}>${escapeHtml(name)}</option>`).join("")}</select>`;
  }
  if (key === "hazardType") {
    return `<select class="select ledger-inline-control revision-inline-control revision-hazard-type" ${base}>${renderGovernanceHazardTypeOptions(hazard.hazardType)}</select>`;
  }
  if (key === "professionalType") {
    return `<select class="select ledger-inline-control revision-inline-control revision-professional-type" ${base}>${renderGovernanceProfessionalTypeOptions(hazard.hazardType, hazard.professionalType)}</select>`;
  }
  if (key === "currentStatus") {
    return `<select class="select ledger-inline-control ledger-inline-status revision-inline-control" ${base}>${settings.statuses.map((name) => `<option ${hazard.currentStatus === name ? "selected" : ""}>${escapeHtml(name)}</option>`).join("")}</select>`;
  }
  if (key === "platformCloseStatus") {
    return `<select class="select ledger-inline-control ledger-inline-platform revision-inline-control" ${base}>${settings.platformCloseStatuses.map((name) => `<option ${hazard.platformCloseStatus === name ? "selected" : ""}>${escapeHtml(name)}</option>`).join("")}</select>`;
  }
  const listAttr = key === "responsibleDept" ? ` list="govResponsibleDeptOptions"` : key === "responsiblePerson" ? ` list="govResponsiblePersonOptions"` : key === "supervisionLeader" ? ` list="govSupervisionLeaderOptions"` : key === "acceptor" ? ` list="govAcceptorOptions"` : "";
  return `<input class="input ledger-inline-control revision-inline-control" ${listAttr} value="${escapeAttr(value || "")}" ${base}>`;
}

function bindGovernanceTaskRevisionEvents(content) {
  const fieldSettings = content.querySelector(".revision-field-settings");
  fieldSettings?.addEventListener("toggle", () => {
    governanceState.revisionFieldSettingsOpen = fieldSettings.open;
  });
  const bulkPanel = content.querySelector(".revision-bulk-panel");
  bulkPanel?.addEventListener("toggle", () => {
    governanceState.revisionBulkOpen = bulkPanel.open;
  });
  content.querySelector("#revisionCheckTypeCode")?.addEventListener("change", (event) => {
    governanceState.revisionCheckTypeCode = event.target.value;
    const first = governanceState.data.inspections.find((item) => item.checkTypeCode === governanceState.revisionCheckTypeCode);
    governanceState.selectedInspectionId = first?.id || "";
    governanceState.revisionSelectedHazardIds = [];
    renderGovernanceActiveTab();
  });
  content.querySelector("#revisionInspectionId")?.addEventListener("change", (event) => {
    governanceState.selectedInspectionId = event.target.value;
    const inspection = governanceState.data.inspections.find((item) => item.id === event.target.value);
    governanceState.revisionCheckTypeCode = inspection?.checkTypeCode || governanceState.revisionCheckTypeCode;
    governanceState.revisionSelectedHazardIds = [];
    renderGovernanceActiveTab();
  });
  content.querySelector("#revisionMarkClosedButton")?.addEventListener("click", markGovernanceRevisionInspectionClosed);
  content.querySelector("#revisionMarkDoneButton")?.addEventListener("click", markGovernanceRevisionInspectionDone);
  content.querySelectorAll("[data-revision-field]").forEach((input) => {
    input.addEventListener("change", () => {
      const fields = Array.from(content.querySelectorAll("[data-revision-field]"))
        .filter((item) => item.checked)
        .map((item) => item.value);
      if (!fields.length) {
        showToast("至少保留一个字段");
        governanceState.revisionFields = GOVERNANCE_REVISION_DEFAULT_FIELDS;
      } else {
        governanceState.revisionFields = fields;
      }
      governanceState.revisionFieldSettingsOpen = true;
      renderGovernanceActiveTab();
    });
  });
  content.querySelector("#revisionBulkHazardType")?.addEventListener("change", (event) => {
    const professional = content.querySelector("#revisionBulkProfessionalType");
    if (professional) {
      professional.innerHTML = `<option value="">不修改</option>${renderGovernanceProfessionalTypeOptions(event.target.value)}`;
    }
  });
  content.querySelectorAll("[data-revision-select]").forEach((button) => {
    button.addEventListener("click", () => updateGovernanceRevisionSelection(button.dataset.revisionSelect));
  });
  content.querySelectorAll("[data-revision-toggle-id]").forEach((button) => {
    button.addEventListener("click", () => toggleGovernanceRevisionHazardSelection(button.dataset.revisionToggleId));
  });
  content.querySelectorAll(".revision-row-select").forEach((input) => {
    input.addEventListener("change", () => toggleGovernanceRevisionHazardSelection(input.value, input.checked));
  });
  content.querySelector("#revisionApplyBulkButton")?.addEventListener("click", applyGovernanceRevisionBulkUpdate);
  content.querySelectorAll(".revision-inline-control").forEach((control) => {
    control.addEventListener("change", () => saveGovernanceTaskRevisionInlineEdit(control));
    if (control.tagName === "INPUT" || control.tagName === "TEXTAREA") {
      control.addEventListener("blur", () => saveGovernanceTaskRevisionInlineEdit(control));
    }
  });
}

function updateGovernanceRevisionSelection(action) {
  const hazards = getGovernanceTaskRevisionHazards();
  const current = new Set(governanceState.revisionSelectedHazardIds || []);
  if (action === "all") {
    governanceState.revisionSelectedHazardIds = hazards.map((item) => item.id);
  } else if (action === "invert") {
    governanceState.revisionSelectedHazardIds = hazards.filter((item) => !current.has(item.id)).map((item) => item.id);
  } else {
    governanceState.revisionSelectedHazardIds = [];
  }
  governanceState.revisionBulkOpen = true;
  renderGovernanceActiveTab();
}

function toggleGovernanceRevisionHazardSelection(hazardId, forceValue) {
  if (!hazardId) return;
  const selected = new Set(governanceState.revisionSelectedHazardIds || []);
  const shouldSelect = typeof forceValue === "boolean" ? forceValue : !selected.has(hazardId);
  if (shouldSelect) selected.add(hazardId);
  else selected.delete(hazardId);
  governanceState.revisionSelectedHazardIds = Array.from(selected);
  governanceState.revisionBulkOpen = true;
  renderGovernanceActiveTab();
}

async function markGovernanceRevisionInspectionClosed() {
  const inspection = getGovernanceTaskRevisionInspection();
  const hazards = getGovernanceTaskRevisionHazards(inspection);
  if (!inspection || !hazards.length) {
    showToast("当前检查没有隐患");
    return;
  }
  if (!window.confirm(`确认将“${inspection.checkNo} ${inspection.checkName}”下 ${hazards.length} 条隐患的平台闭环状态全部修改为“已上传闭环”吗？`)) return;
  const ids = new Set(hazards.map((item) => item.id));
  const today = getTodayDate();
  governanceState.data.hazards = governanceState.data.hazards.map((hazard) => ids.has(hazard.id)
    ? { ...hazard, platformCloseStatus: "已上传闭环", platformUploadDate: hazard.platformUploadDate || today, updatedAt: new Date().toISOString() }
    : hazard);
  await saveGovernanceData();
  showToast("已标记全部上传闭环");
  renderGovernanceActiveTab();
...
async function markGovernanceRevisionInspectionDone() {
  const inspection = getGovernanceTaskRevisionInspection();
  const hazards = getGovernanceTaskRevisionHazards(inspection);
  if (!inspection || !hazards.length) {
    showToast("当前检查没有隐患");
  ...
  const missingText = hazards.filter((item) => !String(item.handlingMeasures || "").trim()).length;
  const missingPhotos = hazards.filter((item) => !(item.afterPhotos || []).length).length;
  const extra = missingText || missingPhotos ? `\n其中 ${missingText} 条未填写处理措施，${missingPhotos} 条未上传整改后照片。` : "";
  if (!window.confirm(`确认将“${inspection.checkNo} ${inspection.checkName}”下 ${hazards.length} 条隐患全部修改为“已整改”吗？${extra}`)) return;
  const ids = new Set(hazards.map((item) => item.id));
  governanceState.data.hazards = governanceState.data.hazards.map((hazard) => {
    if (!ids.has(hazard.id)) return hazard;
    const updated = { ...hazard, currentStatus: "已整改", updatedAt: new Date().toISOString() };
    updated.autoStatus = computeGovernanceAutoStatus(updated);
    return updated;
  });
  await saveGovernanceData();
...
async function applyGovernanceRevisionBulkUpdate() {
  const selectedIds = new Set(governanceState.revisionSelectedHazardIds || []);
  if (!selectedIds.size) {
    showToast("请先选择隐患序号");
    return;
  }
  const values = {};
  document.querySelectorAll("[data-revision-bulk-field]").forEach((field) => {
    const key = field.dataset.revisionBulkField;
  ...
  if (!window.confirm(`确认将已填写的批量字段应用到 ${selectedIds.size} 条隐患吗？`)) return;
  governanceState.data.hazards = governanceState.data.hazards.map((hazard) => {
    if (!selectedIds.has(hazard.id)) return hazard;
    const updated = { ...hazard, ...values, updatedAt: new Date().toISOString() };
    if (values.hazardType && !values.professionalType) {
      updated.professionalType = (governanceState.data.settings.hazardTypes[values.hazardType] || [])[0] || "";
    }
    updated.autoStatus = computeGovernanceAutoStatus(updated);
    return updated;
  });
...
async function saveGovernanceTaskRevisionInlineEdit(control) {
  const hazardId = control.dataset.revisionHazardId;
  const field = control.dataset.revisionInlineField;
  const hazard = governanceState.data.hazards.find((item) => item.id === hazardId);
  if (!hazard || !field) return;
  ...
  if (field === "hazardType") {
    const professionals = governanceState.data.settings.hazardTypes[value] || [];
    if (!professionals.includes(updated.professionalType)) {
      updated.professionalType = professionals[0] || "";
    }
  }
  updated.autoStatus = computeGovernanceAutoStatus(updated);
  governanceState.data.hazards = governanceState.data.hazards.map((item) => item.id === hazardId ? updated : item);
...
async function previewGovernanceFeedbackTextCode() {
  const input = document.querySelector("#feedbackTextCodeInput");
  const panel = document.querySelector("#feedbackTextCodePreviewPanel");
  try {
    const decoded = await decodeHazardTextCode(input?.value || "");
    validateHazardImportData(decoded.data, ["hazard-feedback"]);
    const preview = previewImportData(decoded.data);
    panel.innerHTML = renderTextCodePreview(preview, {
      confirmAction: "governance-feedback",
      cancelAction: "governance-feedback",
      rawData: decoded.data
    });
    panel.querySelector("[data-text-code-confirm]").addEventListener("click", () => {
      governanceState.feedbackImportDraft = buildFeedbackImportReviewDraft(decoded.data.hazards.map((feedback, index) => ({
        id: createId(),
        fileName: "微信数据码",
        sourceDept: decoded.data.responsibleDept || "",
        sourceIndex: index + 1,
        feedback
      })));
      renderGovernanceFeedbackReview();
      showToast("数据码已解析，请核对后提交导入台账");
    });
    panel.querySelector("[data-text-code-cancel]").addEventListener("click", () => {
      panel.innerHTML = "";
    });
...
async function exportGovernanceWordReport(type) {
  ...
  for (let index = 0; index < hazards.length; index += 1) {
    ...
    children.push(reportHazardTitleParagraph(d, ensureChinesePeriod(hazard.problem), templateContext.rectificationTitleStyleId || templateContext.bodyStyleId));
...
function renderGovernanceBackupPage(content) {
    ...
    <section class="tool-card glass">
      ...
      <div class="report-stage-grid">
        <article class="report-stage-card">
          ...
          <div class="actions">
            <button id="copyGovAllTextCodeButton" class="button primary" type="button">复制微信数据码</button>
            <button id="viewGovAllTextCodeButton" class="button" type="button">查看数据码</button>
            <button id="exportGovAllButton" class="button" type="button">下载 JSON 文件</button>
            <button class="button ghost" type="button" disabled>生成二维码（预留）</button>
          </div>
        </article>
        <article class="report-stage-card">
          <span>整体导入</span>
          <h3>粘贴微信数据码恢复</h3>
          <p>推荐方式：从微信复制整体备份数据码，粘贴后先预览，再确认覆盖当前年度汇总台账。</p>
          <textarea id="govAllTextCodeInput" class="textarea text-code-input" placeholder="请粘贴微信中收到的数据码，格式一般为：&#10;【隐患治理备份数据】&#10;HAZARD-DATA:v1:......"></textarea>
          <div class="actions">
            <button id="parseGovAllTextCodeButton" class="button primary" type="button">识别数据码</button>
            <button id="clearGovAllTextCodeButton" class="button ghost" type="button">清空</button>
          </div>
          <div id="govAllTextCodePreviewPanel" class="text-code-preview-panel"></div>
        </article>
        <article class="report-stage-card">
          <span>备用导入</span>
          <h3>从备份 JSON 恢复</h3>
          <p>适合电脑端或能方便找到文件的手机环境。导入后会覆盖当前年度汇总台账。</p>
  ...
  content.querySelector("#copyGovAllTextCodeButton").addEventListener("click", copyGovernanceFullBackupTextCode);
  content.querySelector("#viewGovAllTextCodeButton").addEventListener("click", viewGovernanceFullBackupTextCode);
  content.querySelector("#parseGovAllTextCodeButton").addEventListener("click", previewGovernanceFullBackupTextCode);
  content.querySelector("#clearGovAllTextCodeButton").addEventListener("click", () => {
    content.querySelector("#govAllTextCodeInput").value = "";
    content.querySelector("#govAllTextCodePreviewPanel").innerHTML = "";
  });
...
async function copyGovernanceFullBackupTextCode() {
  recalculateGovernanceData();
  try {
    await copyHazardTextCode(buildGovernanceFullBackupPayload(), { dataType: "governance-full-backup", label: "隐患治理备份数据" });
  } catch (error) {
    showToast(error.message || "数据码复制失败");
  }
}

async function viewGovernanceFullBackupTextCode() {
  recalculateGovernanceData();
  try {
    await showHazardTextCodeDialog(buildGovernanceFullBackupPayload(), { title: "整体备份微信数据码", dataType: "governance-full-backup", label: "隐患治理备份数据" });
  } catch (error) {
    showToast(error.message || "数据码生成失败");
  }
}

async function previewGovernanceFullBackupTextCode() {
  const input = document.querySelector("#govAllTextCodeInput");
  const panel = document.querySelector("#govAllTextCodePreviewPanel");
  try {
    const decoded = await decodeHazardTextCode(input?.value || "");
    validateHazardImportData(decoded.data, ["governance-full-backup"]);
    const parsed = parseGovernanceFullBackupPayload(decoded.data);
    const preview = {
      checkName: "整体备份",
      checkNo: "全部数据",
      checkDate: decoded.data.exportedAt || decoded.data.exportTime || "",
      checkPlace: "年度汇总工作台",
      checkType: "整体备份",
      responsibleDept: "",
      total: parsed.data.hazards.length,
      done: parsed.data.hazards.filter((hazard) => hazard.currentStatus === "已整改" || hazard.autoStatus === "已整改").length,
      unfinished: parsed.data.hazards.filter((hazard) => hazard.currentStatus !== "已整改" && hazard.autoStatus !== "已整改").length,
      delayed: parsed.data.hazards.filter((hazard) => hazard.currentStatus === "延期整改" || hazard.autoStatus === "延期整改").length,
      overdue: parsed.data.hazards.filter((hazard) => hazard.autoStatus === "已逾期").length,
      source: "文本数据码",
      version: "v1"
    };
    panel.innerHTML = renderTextCodePreview(preview, {
      confirmAction: "governance-full-backup",
      cancelAction: "governance-full-backup",
      rawData: {
        inspections: parsed.data.inspections.length,
        hazards: parsed.data.hazards.length,
        settings: Object.keys(parsed.data.settings?.hazardTypes || {}).length,
        preferences: parsed.preferences
      }
    });
    panel.querySelector("[data-text-code-confirm]").addEventListener("click", async () => {
      if (!window.confirm(`确认用数据码中的 ${parsed.data.inspections.length} 项检查、${parsed.data.hazards.length} 条隐患覆盖当前年度汇总台账吗？`)) return;
    ...
    <div class="tool-layout hazard-tool">
      ...
      <section class="toolbar glass">
        ...
        <div class="data-actions hazard-actions">
          ...
          <button id="copyHazardTextCodeButton" class="button primary" type="button">复制数据码</button>
          <button id="viewHazardTextCodeButton" class="button" type="button">查看数据码</button>
          <button class="button ghost" type="button" disabled>生成二维码（预留）</button>
          <button id="exportHazardJsonButton" class="button" type="button">下载JSON</button>
          <button id="importHazardJsonButton" class="button" type="button">导入JSON</button>
      ...
      <section class="tool-card glass text-code-panel">
        <div class="section-title">
          <div>
            <h3>推荐方式：粘贴微信数据码</h3>
            <p class="hint">从微信复制检查隐患数据码，粘贴后先预览，再进入原有合并确认流程。备用方式仍可选择 JSON 文件导入。</p>
          </div>
        </div>
        <label class="field span-2">
          <span>检查隐患数据码</span>
          <textarea id="hazardTextCodeInput" class="textarea text-code-input" placeholder="请粘贴微信中收到的隐患数据码，格式一般为：&#10;【隐患检查数据】&#10;HAZARD-DATA:v1:......"></textarea>
        </label>
        <div class="actions">
          <button id="parseHazardTextCodeButton" class="button primary" type="button">识别数据码</button>
          <button id="clearHazardTextCodeButton" class="button ghost" type="button">清空</button>
        </div>
        <div id="hazardTextCodePreviewPanel" class="text-code-preview-panel"></div>
      </section>
...
function bindHazardEvents() {
  ...
  document.querySelector("#copyHazardTextCodeButton").addEventListener("click", copyHazardInspectionTextCode);
  document.querySelector("#viewHazardTextCodeButton").addEventListener("click", viewHazardInspectionTextCode);
  document.querySelector("#parseHazardTextCodeButton").addEventListener("click", previewHazardInspectionTextCode);
  document.querySelector("#clearHazardTextCodeButton").addEventListener("click", () => {
    document.querySelector("#hazardTextCodeInput").value = "";
    document.querySelector("#hazardTextCodePreviewPanel").innerHTML = "";
  });
...
async function copyHazardInspectionTextCode() {
  try {
    await copyHazardTextCode(hazardState.data, { dataType: "inspection-create", label: "隐患检查数据" });
  } catch (error) {
    showToast(error.message || "数据码复制失败");
  }
}

async function viewHazardInspectionTextCode() {
  try {
    await showHazardTextCodeDialog(hazardState.data, { title: "检查隐患微信数据码", dataType: "inspection-create", label: "隐患检查数据" });
  } catch (error) {
    showToast(error.message || "数据码生成失败");
  }
}

async function previewHazardInspectionTextCode() {
  const input = document.querySelector("#hazardTextCodeInput");
  const panel = document.querySelector("#hazardTextCodePreviewPanel");
  try {
    const decoded = await decodeHazardTextCode(input?.value || "");
    validateHazardImportData(decoded.data, ["inspection-create"]);
    if (!isValidHazardData(decoded.data)) {
      throw new Error("数据中缺少 checkInfo 或 hazards，无法导入。");
    }
    const preview = previewImportData(decoded.data);
    panel.innerHTML = renderTextCodePreview(preview, {
      confirmAction: "hazard-inspection",
      cancelAction: "hazard-inspection",
      rawData: decoded.data
    });
    panel.querySelector("[data-text-code-confirm]").addEventListener("click", () => {
      openMergePanel([{ fileName: "微信数据码", data: normalizeHazardData(decoded.data) }]);
      showToast("数据码已解析，请在下方合并确认后导入");
    });
    panel.querySelector("[data-text-code-cancel]").addEventListener("click", () => {
      panel.innerHTML = "";
    });
